...
void calcAns(Dir* root, int spaceNeeded) {
    if(root->size >= spaceNeeded && root->size <= ans) ans = root->size;
    for(int i = 0; i < root->children.size(); ++i) calcAns(root->children[i],spaceNeeded);
}
...
int main() {
    ...
    int availSpace = 70000000 - totalSize;
    int spaceNeeded = 30000000 - availSpace;

    calcAns(root,spaceNeeded);
    cout<<ans<<endl;
}